...
# 広域連系系統のマスタープラン及び系統利用ルールの在り方等に関する検討委員会の関係資料一覧のhtml作成
...
<html>
<head>
  <title>広域連系系統のマスタープラン及び系統利用ルールの在り方等に関する検討委員会</title>
  <meta charset="UTF-8">
</head>
<body>
{body}
...
# 見出し1
body = '''<h1>広域連系系統のマスタープラン及び系統利用ルールの在り方等に関する検討委員会</h1>
<a href="https://www.occto.or.jp/iinkai/masutapuran/index.html" target="_blank">委員会ページ</a>'''

# 情報を取得する対象ページ（今年度）
name_url = 'https://www.occto.or.jp/iinkai/masutapuran/index.html'
...
if res.status_code == 404: # ページ接続出来なかった場合
    ...
    for c in soup.find('main', {'id': 'contents'}).find('h1').next_siblings:
        if type(c) is bs4.element.Tag: 
            if (c.name == 'h2') and (c.get('id') == 'konzatsu'): # 地内混雑勉強会の見出しになったら終了
                break
