...
# 地内系統の混雑管理に関する勉強会の関係資料一覧のhtml作成
...
# 定数定義
# -----------------------------------
NAME_HTML = '地内系統の混雑管理に関する勉強会.html'
DIR_OUTPUT = r'../occto'
...
# 見出し1
body = '''<h1>地内系統の混雑管理に関する勉強会</h1>
<a href="https://www.occto.or.jp/iinkai/masutapuran/index.html#konzatsu" target="_blank">委員会ページ</a>'''

# 情報を取得する対象ページ（今年度）
name_url = 'https://www.occto.or.jp/iinkai/masutapuran/index.html#konzatsu'
...
if res.status_code == 404: # ページ接続出来なかった場合
    ...
    for c in soup.find('main', {'id': 'contents'}).find('h2', {'id': 'konzatsu'}).next_siblings:
        if type(c) is bs4.element.Tag: 
            if (c.name == 'h2') and (c.get('id') == 'cost'): # 最後まで、左は意味なし
                break
