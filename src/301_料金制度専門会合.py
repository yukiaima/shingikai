...
## 開催回・資料リンク先の取得
name_url = 'https://www.emsc.meti.go.jp/activity/index_electricity.html'
...
if res.status_code == 404: # ページ接続出来なかった場合
    ...
    # 回ごとに処理（料金審査専門会合）
    for tr in soup.find('table', {'class': 'tableLayout borderdot', 'summary': '電気料金審査専門会合 開催一覧'}).find_all('tr'):
        # 開催回ごとのhtml文
        body_n_com = '''
        <h2>{title}</h2>
        <ul>{papers}</ul>
    ...
    # 回ごとに処理（料金制度専門会合）
    for tr in soup.find('table', {'class': 'tableLayout borderdot', 'summary': '電気料金制度専門会合 開催一覧'}).find_all('tr'):
        # 開催回ごとのhtml文
        body_n_com = '''
        <h2>{title}</h2>
        <ul>{papers}</ul>
        ...
        for td in tr.find_all('td'):
            if (td.get_text(strip=True) != '配付資料'): # 配布資料以外 ※「付」になっている
                if ''.join([a.get('href') for a in td.find_all('a')]) == '': # urlなし
                    title += td.get_text(strip=True)
