...
# 容量市場の在り方等に関する検討会・勉強会の関係資料一覧のhtml作成
...
# 定数定義
# -----------------------------------
NAME_HTML = '容量市場の在り方等に関する検討会・勉強会.html'
DIR_OUTPUT = r'../occto'
...
<html>
<head>
  <title>容量市場の在り方等に関する検討会・勉強会</title>
  <meta charset="UTF-8">
</head>
<body>
{body}
...
# 見出し1
body = '''<h1>容量市場の在り方等に関する検討会・勉強会</h1>
<a href="https://www.occto.or.jp/iinkai/youryou/index.html" target="_blank">委員会ページ</a>'''

# 情報を取得する対象ページ（今年度）
name_url = 'https://www.occto.or.jp/iinkai/youryou/index.html'
...
if res.status_code == 404: # ページ接続出来なかった場合
    ...
    for c in soup.find('main', {'id': 'contents'}).find('h1').next_siblings:
        if type(c) is bs4.element.Tag: 
            if (c.name == 'h2') and (c.get('id') == 'study'): # 勉強会の見出しになったら止める
                break
            elif (c.name == 'ul') and (c.get('class') == ['list2', 'index']): # リストの場合
                for li in c.find_all('li'):
                    if li.get_text(strip=True)[-5:] == '年度開催分': # 過去分
                        # 過去分のurl
