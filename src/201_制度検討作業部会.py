...
URL_COMMITTEE = 'https://www.meti.go.jp/shingikai/enecho/denryoku_gas/jisedai_kiban/system_review/index.html'
...
if os.path.isfile(r'{}\{}'.format(DIR_OUTPUT, NAME_HTML)): 
    ...
    if soup_old.find('div', {'id': 'update'}) != None:
        # 最終更新日が一致する場合は、処理不要。一致しない場合は、更新日を更新して処理
        if soup_old.find('div', {'id': 'update'}).get_text(strip=True) == soup.find('div', {'id': '__rdo_update'}).get_text(strip=True):
            flag_proceed = False
        else:
            body = body.format(name_committee = NAME_COMMITTEE, url = URL_COMMITTEE, 
                               update = soup.find('div', {'id': '__rdo_update'}).get_text(strip=True))

# 更新の場合、更新
if flag_proceed == True:
    ## 開催回・資料リンク先の取得
    name_url_list = ['https://www.meti.go.jp/shingikai/enecho/denryoku_gas/jisedai_kiban/system_review/index.html', # 直近
