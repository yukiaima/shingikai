...
# 見出し1
body = '''<h1>広域系統整備委員会</h1>
<a href="https://www.occto.or.jp/iinkai/kouikikeitouseibi/index.html" target="_blank">委員会ページ</a>'''

# 情報を取得する対象ページ（今年度）
name_url = 'https://www.occto.or.jp/iinkai/kouikikeitouseibi/index.html'
...
if res.status_code == 404: # ページ接続出来なかった場合
    ...
    for c in soup.find('main', {'id': 'contents'}).find('h1').next_siblings:
        if type(c) is bs4.element.Tag: 
            if (c.name == 'h2') and (c.get('id') == 'resilience'): # レジ小委の見出しになったら終了
                break
