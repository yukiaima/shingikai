...
# 定数定義
# -----------------------------------
NAME_COMMITTEE = 'GX実行会議'
URL_COMMITTEE = 'https://www.cas.go.jp/jp/seisaku/gx_jikkou_kaigi/index.html'
        ...
        if (td.get_text(strip=True) != '議事次第・資料'): # 配布資料以外
            ...
            html_papers += '\n'.join(
                ['<li><a href={} target="_blank">{}</a></li>'.format(
                    ...
                    ) for tr2 in soup_papers.find('table', {'class', 'tbl_giji_nl'}).find_all('tr') if len(tr2.find_all('a'))>0])
