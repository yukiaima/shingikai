...
# 将来の運用容量等の在り方に関する作業会の関係資料一覧のhtml作成
...
# 定数定義
# -----------------------------------
NAME_HTML = '将来の運用容量等の在り方に関する作業会.html'
DIR_OUTPUT = r'../occto'
...
<html>
<head>
  <title>将来の運用容量等の在り方に関する作業会</title>
  <meta charset="UTF-8">
</head>
<body>
{body}
...
# 見出し1
body = '''<h1>将来の運用容量等の在り方に関する作業会</h1>
<a href="https://www.occto.or.jp/iinkai/chouseiryoku/index.html#unyouyouryousagyoukai" target="_blank">委員会ページ</a>'''

# 情報を取得する対象ページ（今年度）
name_url = 'https://www.occto.or.jp/iinkai/chouseiryoku/index.html#unyouyouryousagyoukai'
...
if res.status_code == 404: # ページ接続出来なかった場合
    ...
    # 将来の運用容量等の在り方に関する作業会に関係する部分から探索し、今年度に該当する部分を取得
    for c in soup.find('main', {'id': 'contents'}).find('h2', {'id': 'unyouyouryousagyoukai'}).next_siblings:
        if type(c) is bs4.element.Tag: 
            if (c.name == 'h2') and (c.get('id') == 'jizoku'): # 持続的需要変動に関する勉強会の見出しになったら終了
                break
