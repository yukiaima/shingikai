...
# ガス事業制度検討ワーキンググループの関係資料一覧のhtml作成
...
# 定数定義
# -----------------------------------
NAME_HTML = 'ガス事業制度検討ワーキンググループ.html'
DIR_OUTPUT = r'../meti'
...
# 見出し1
body = '''<h1>ガス事業制度検討ワーキンググループ</h1>
<a href="https://www.meti.go.jp/shingikai/enecho/denryoku_gas/denryoku_gas/gas_jigyo_wg/index.html" target="_blank">委員会ページ</a>'''

## 開催回・資料リンク先の取得
name_url = 'https://www.meti.go.jp/shingikai/enecho/denryoku_gas/denryoku_gas/gas_jigyo_wg/index.html'
...
if res.status_code == 404: # ページ接続出来なかった場合
    ...
    for ul in soup.find('div', {'id': '__main_contents', 'class': 'main w1000'}).find_all('ul'):
        if (ul.get('class') == ['linkE', 'clearfix']) or (ul.get('class') == ['linkD', 'clearfix']):
