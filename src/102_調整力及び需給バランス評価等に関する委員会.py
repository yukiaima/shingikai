...
# 調整力及び需給バランス評価等に関する委員会の関係資料一覧のhtml作成
...
# 定数定義
# -----------------------------------
NAME_HTML = '調整力及び需給バランス評価等に関する委員会.html'
DIR_OUTPUT = r'../occto'
...
<html>
<head>
  <title>調整力及び需給バランス評価等に関する委員会</title>
  <meta charset="UTF-8">
</head>
<body>
{body}
...
# 見出し1
body = '''<h1>調整力及び需給バランス評価等に関する委員会</h1>
<a href="https://www.occto.or.jp/iinkai/chouseiryoku/index.html" target="_blank">委員会ページ</a>'''

# 情報を取得する対象ページ（今年度）
name_url = 'https://www.occto.or.jp/iinkai/chouseiryoku/index.html'
...
if res.status_code == 404: # ページ接続出来なかった場合
    ...
    for c in soup.find('main', {'id': 'contents'}).find('h1').next_siblings:
        if type(c) is bs4.element.Tag: 
            if (c.name == 'h2') and (c.get('id') == 'jukyuchousei'): # 需給調整市場検討小委員会の見出しになったら終了
                break
